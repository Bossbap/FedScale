...
def process_cmd(yaml_file, local=False):
    ...
    with open(f"{job_name}_logging2", 'wb') as fout:
        pass

    with open(f"{job_name}_logging2", 'a') as fout:
        if local:
    ...
    print(f"if you cannot find logs directory on the path, you need to check ""{job_name}_logging2"" log file under FEDSCALE root directory.")
